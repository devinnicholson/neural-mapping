...
class SplitGenerationTests(unittest.TestCase):
    ...
    def test_active_score_pose_hybrid_balances_score_and_pose(self) -> None:
        ...
        positions = {
            "frame_000.png": (0.0, 0.0, 0.0),
            "frame_001.png": (1.0, 0.0, 0.0),
            "frame_002.png": (1.1, 0.0, 0.0),
            "frame_003.png": (8.0, 0.0, 0.0),
            "frame_004.png": (10.0, 0.0, 0.0),
        }
        scores = {
            "frame_002.png": 1.0,
            "frame_003.png": 0.7,
        ...
        order = active_score_pose_hybrid_order(
        ...
        self.assertEqual(order[0], "frame_003.png")
    ...
    def test_generate_active_split_cli_accepts_frame_report_scores(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            transforms = root / "transforms.json"
            transforms.write_text(
            ...
            scores = root / "ensemble_report.json"
            scores.write_text(
                json.dumps(
                    {
                        "metadata": {"uncertainty_signal": "ensemble-rgb-variance"},
                        "frames": [
                            {"file_path": "frame_004.png", "mean_uncertainty": 0.1},
                            {"file_path": "frame_005.png", "mean_uncertainty": 0.9},
                        ],
                    }
                ),
                encoding="utf-8",
    ...
    def test_generate_active_split_cli_rank_averages_multiple_score_keys(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            transforms = root / "transforms.json"
            transforms.write_text(
                        ...
                        "frames": [
                            {
                                ...
                                "top_decile_mean_uncertainty": {
                                    "transmittance": 0.9,
                                    "local-mean-transmittance": 0.2,
                                    "accumulation-gradient": 0.1,
                                },
                            },
                            {
                                "file_path": "frame_005.png",
                                "top_decile_mean_uncertainty": {
                                    "transmittance": 0.8,
                                    "local-mean-transmittance": 0.9,
                                    "accumulation-gradient": 0.9,
                                },
                            },
                            {
                                "file_path": "frame_006.png",
                                "top_decile_mean_uncertainty": {
                                    "transmittance": 0.1,
                                    "local-mean-transmittance": 0.1,
                                    "accumulation-gradient": 0.2,
                                },
                            },
                        ],
                    }
            ...
            self.assertIn("frame_005.png", payload["splits"]["3"]["train"])
            self.assertNotIn("frame_004.png", payload["splits"]["3"]["train"])
            self.assertNotIn("frame_006.png", payload["splits"]["3"]["train"])
    ...
    def test_generate_active_split_cli_accepts_score_pose_hybrid(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            payload = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(payload["selection_method"], "active-score-pose-hybrid")
            self.assertIn("frame_005.png", payload["splits"]["3"]["train"])
